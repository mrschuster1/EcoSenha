unit EcoSenha.Components.CustomCalendar;
...
type
  ...
  public
    ...
    property SelectedDayColor: Cardinal read FSelectedDayColor
      write FSelectedDayColor;
...
begin
  ...
  begin
    ...
    lblDay.StyledSettings := lblDay.StyledSettings - [TStyledSetting.Size,
      TStyledSetting.FontColor];
    lblDay.FontColor := $FFBDBDBD;
    lblDay.Font.Size := FDayFontSize;
    lblDay.Font.Family := 'Lexend';
  ...
  begin
    ...
    lblDay.StyledSettings := lblDay.StyledSettings - [TStyledSetting.Style,
...
procedure TCustomCalendar.AddMarker(day: integer;
  TextColor: TAlphaColor = $FFFFFFFF;
  BackgroundColor: TAlphaColor = $FFE68B93);
var
  c: TCircle;
  lbl: TLabel;
...
begin
  ...
  begin
    ...
    if x <= MonthDays[IsLeapYear(YearOf(FSelectedDate)), MonthOf(FSelectedDate)]
    then
    begin
      WeekDay := DayOfWeek(EncodeDate(YearOf(FSelectedDate),
        MonthOf(FSelectedDate), x));

      if (WeekDay = 7) then
        inc(cont)
      else
        if (WeekDay <> 7) and (x = MonthDays[IsLeapYear(YearOf(FSelectedDate)),
        MonthOf(FSelectedDate)]) then
        inc(cont);
    end;
  ...
  for x := 1 to 7 do
  begin
    rectDay := rectCalendar.FindComponent('day_name' + x.ToString)
      as TRectangle;
    rectDay.Fill.Color := FBackgroundColor;
